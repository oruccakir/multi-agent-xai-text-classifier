...
The margin over the second-best model (always SVM) ranges from 1.2 points (IMDB) to 1.9
points (Turkish News).
...
macro F1 above 93\%, which means the negative class is a real problem across the board.
...
underlying model: for NB, LR, and SVM, the SHAP feature importances correlate directly
with TF-IDF weights and are easy to verify manually, while for the transformer the
explanations reflect a much more complex, non-linear decision function and may be less
...
sentiment and topic tasks based on training examples from each dataset. The modular design
...
\begin{itemize}
  ...
  \item \textbf{(RQ part 2)} Both SHAP (\texttt{shap.Explainer}) and LIME
        (\texttt{LimeTextExplainer}) are applied uniformly across all models using a
        model-agnostic interface. Classical models (NB, LR, SVM) produce explanations
        that correlate directly with TF-IDF term weights and are easy to verify, while
        transformer explanations reflect a more complex decision boundary and are harder
