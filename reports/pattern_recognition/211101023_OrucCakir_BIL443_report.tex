...
I apply the following preprocessing steps (\texttt{src/preprocessing/text\_preprocessor.py}):
...
I use TF-IDF vectorization to produce a 30,000-dimensional sparse feature vector per
...
To visualize the feature space, I applied PCA to 50 dimensions and then t-SNE
...
\begin{figure}[htbp]
	\centerline{\includegraphics[width=\columnwidth]{turkish_sentiment/tsne_2d.png}}
...
I also ran K-Means clustering ($k=3$, \texttt{random\_state=42}) on the PCA-50
...
\begin{figure}[htbp]
	\centerline{\includegraphics[width=\columnwidth]{turkish_sentiment/kmeans_clusters.png}}
...
I trained and evaluated seven classical classifiers, all using a TF-IDF pipeline
with \texttt{norm='l2'}:
...
\textbf{XGBoost} (200 trees, \texttt{max\_depth=5}, \texttt{learning\_rate=0.05}):
...
other classical classifier. SVM's advantage stems from three properties of LinearSVC:
(1) it directly optimizes the margin in the high-dimensional TF-IDF space where linear
separability is good; (2) it is not sensitive to feature independence assumptions
(unlike Naive Bayes); and (3) L2 normalization stabilizes the margin even under the
unequal class distribution. Logistic Regression, despite being a linear model like SVM,
uses the SAGA solver on 10K samples (a setting optimized for larger data), causing
convergence variance across seeds that widens its performance gap.
